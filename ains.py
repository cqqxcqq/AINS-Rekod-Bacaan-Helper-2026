...
class Config:
    ...
    json_files: Dict[str, str] = field(
        default_factory=lambda: {
            "malay": "book_malay.json",
            "english": "book_english.json",
            "other": "book_other.json",
        }
    )
    ...
    daily_limit: int = 30  # AINS max 30 book entries per day
...
class LanguageDetector:
    ...
    # Maps our internal language names -> what to select in the AINS dropdown.
    # The AINS "Bahasa" dropdown has exactly:
    #   "Bahasa Melayu", "English", "Lain-lain"
    AINS_DROPDOWN_MAP = {
        "Bahasa Melayu": "Bahasa Melayu",
        "English": "English",
        "Bahasa Cina": "Lain-lain",
        "Bahasa Tamil": "Lain-lain",
        "Bahasa Arab": "Lain-lain",
        "Lain-lain": "Lain-lain",
    }

    @classmethod
    def get_ains_dropdown_value(cls, detected_language: str) -> str:
        """Convert detected language to the value available in AINS dropdown."""
        return cls.AINS_DROPDOWN_MAP.get(detected_language, "Lain-lain")

    @classmethod
    def detect(cls, book: Dict, default: str = "English") -> str:
        """Detect language from book data."""
        # Check _source_lang first (set during JSON file loading)
        if "_source_lang" in book and book["_source_lang"]:
            source_lang = str(book["_source_lang"]).lower().strip()
            if source_lang in cls.SOURCE_LANG_MAP:
                return cls.SOURCE_LANG_MAP[source_lang]
...
class DuplicateTracker:
    ...
    def mark_as_processed(self, book: Dict, index: int):
        ...
        self.duplicates["book_signatures"][signature] = {
            ...
            "timestamp": datetime.now(UTC).isoformat() + "Z",  # UTC time
        }
        self.save()
...
class ProgressTracker:
        ...
        if os.path.exists(self.progress_path):
            try:
                ...
                try:
                    ...
                    shutil.copy2(self.progress_path, backup)
                    self.logger.warning(
                        f"Progress file corrupted! Backup saved to: {backup}"
                    )
                except Exception:
                    self.logger.warning(
                        "Progress file corrupted, starting fresh (no backup)"
                    )
    ...
    def save(self):
        ...
        # Update customer's last_completed_index in customers.json
        if self.config.customer_id:
            try:
                customers_data = load_customers(self.config)
                cust = customers_data.get("customers", {}).get(
                    self.config.customer_id, {}
                )
                cust["last_completed_index"] = self.data.get("last_completed_index", -1)
                customers_data["customers"][self.config.customer_id] = cust
                save_customers(self.config, customers_data)
            except Exception:
                pass
    ...
    def mark_book(self, index: int, title: str, status: BookStatus, error: str = None):
        ...
        if previous_status != status.value:
            ...
            if previous_status:
                try:
                    ...
                    if prev_enum == BookStatus.SUCCESS:
                        self.data["stats"]["success"] = max(
                            0, self.data["stats"]["success"] - 1
                        )
                    elif prev_enum == BookStatus.FAILED:
                        self.data["stats"]["failed"] = max(
                            0, self.data["stats"]["failed"] - 1
                        )
                    elif prev_enum == BookStatus.SKIPPED:
                        self.data["stats"]["skipped"] = max(
                            0, self.data["stats"]["skipped"] - 1
                        )
                    elif prev_enum == BookStatus.DUPLICATE:
                        self.data["stats"]["duplicate"] = max(
                            0, self.data["stats"]["duplicate"] - 1
                        )
                except ValueError:
                    pass
            ...
            if status == BookStatus.SUCCESS:
                # Only advance last_completed_index contiguously (don't skip over failures)
                if index == self.data["last_completed_index"] + 1:
    ...
    def reset_keep_submitted(self):
        ...
        self.save()
        self.logger.info(
            "📍 Start index reset to beginning (submitted books will be skipped)"
        )
...
class AIDuplicateDetector:
    ...
    def __init__(
        self, client: Groq, model: str, logger: logging.Logger, timeout: float = 30.0
    ):
        self.client = client
        self.model = model
        self.logger = logger
...
"""
                ...
                if result:
                    ...
                    if is_dup and confidence > 0.7:
                        self.logger.info(
                            f"   🤖 AI detected duplicate (conf: {confidence:.2f}): {reason}"
                        )
                        return True, reason
...
class AIErrorAnalyzer:
    ...
    def analyze_error(
        self, page: Page, error_message: str, expected_action: str, current_phase: str
    ) -> Dict[str, Any]:
        """
...
"""
            ...
            if result:
                self.logger.info(
                    f"   🔍 Problem: {result.get('problem_detected', 'Unknown')}"
                )
                self.logger.info(
                    f"   💡 Cause: {result.get('likely_cause', 'Unknown')}"
                )
                self.logger.info(f"   ⚠️  Severity: {result.get('severity', 'unknown')}")

                if result.get("recovery_possible"):
                    self.logger.info(
                        f"   ✅ Recovery possible: {len(result.get('recovery_actions', []))} steps"
                    )
                else:
            ...
            return {
                ...
                "problem_detected": "AI analysis failed",
            }
        ...
        try:
            ...
            try:
                ...
                if swal_visible:
                    ...
                    if (
                        "pasti" in swal_text
                        or "confirm" in swal_text
                        or "adakah anda pasti" in swal_text
                    ):
            ...
            input_count = page.locator(
                "input[type='text'], input:not([type='radio']):not([type='hidden']):not([type='checkbox']), textarea"
            ).count()

            logger.debug(
                f"   Simple detection - Radios: {radio_count}, Inputs: {input_count}"
            )

            # Check for favorites page
            if "tambah ke senarai kegemaran" in page_text or (
                "senarai kegemaran" in page_text and "tambah" in page_text
            ):
            ...
            if any(
                word in page_text
                for word in ["berjaya", "successfully", "telah berjaya", "rekod telah"]
            ):
                return FormPhase.SUCCESS

            # Check for book type selection
            if "pilih sumber bacaan" in page_text or (
                "jenis koleksi" in page_text and radio_count > 0
            ):
                return FormPhase.BOOK_TYPE_SELECT

            if radio_count > 0:
                if "buku" in page_text and (
                    "e-buku" in page_text or "ebuku" in page_text
                ):
                    return FormPhase.BOOK_TYPE_SELECT
            ...
            if input_count > 0:
                if any(
                    word in page_text
                    for word in ["tajuk", "penulis", "penerbit", "pengarang"]
                ):
                ...
                if any(
                    word in page_text
                    for word in ["rumusan", "sinopsis", "pengajaran", "nilai murni"]
                ):
...
"""
            ...
            if result and "phase" in result:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"🤖 AI detected: {phase.value} (conf: {result.get('confidence', 0):.2f})"
                    )
                    return phase, result
                except KeyError:
...
class BookAutomation:
    ...
    def initialize_ai(self) -> bool:
        ...
        try:
            self.client = Groq(
                api_key=self.config.groq_api_key, timeout=self.config.groq_timeout
            )
            ...
            if (
                ...
                self.logger.error(
                    f"❌ AI connection timed out after {self.config.groq_timeout}s: {e}"
                )
                self.logger.error(
                    "   Try increasing 'groq_timeout' in Config or check your network."
                )
    ...
    def _normalize_category(self, category: str) -> str:
        """Normalize category text to match dropdown options."""
        if not category:
            return category
        ...
        mapping = {
            "fiksi": "Fiksyen",
            "fiksyen": "Fiksyen",
            "bukan fiksyen": "Bukan Fiksyen",
        }
        return mapping.get(lower, category)

    def load_books(self, json_file_override: str = None) -> List[Dict]:
        """Load books from JSON file. Accepts optional override path."""
        json_filename = json_file_override or self.config.json_file
        json_path = self.config.get_path(json_filename)
        ...
        try:
            ...
            self.logger.info(
                f"📚 Loaded {len(valid_books)}/{len(books)} valid books from {json_filename}"
            )
            return valid_books
    ...
    def load_books_multi(self) -> List[Dict]:
        """Load books from all configured JSON files, merging them."""
        all_books = []
        files_loaded = []

        for lang_key, filename in self.config.json_files.items():
            filepath = self.config.get_path(filename)
            if os.path.exists(filepath):
                try:
                    ...
                    for i, book in enumerate(books):
                        if self._validate_book(book, i):
                            # Tag each book with its source file for debugging
                            book["_source_file"] = filename
                            book["_source_lang"] = lang_key
                            all_books.append(book)
                            valid_count += 1

                    files_loaded.append(f"{filename} ({valid_count} books)")
                    self.logger.info(f"   📄 {filename}: {valid_count} valid books")

                except json.JSONDecodeError as e:
                    self.logger.error(f"   ❌ Invalid JSON in {filename}: {e}")
            else:
                self.logger.debug(f"   📄 {filename}: not found (skipped)")

        self.total_books = len(all_books)
        self.logger.info(
            f"📚 Total loaded: {len(all_books)} books from {len(files_loaded)} file(s)"
        )
        return all_books
    ...
    def take_screenshot(self, name: str):
        ...
        try:
            ...
            filepath = self.config.get_path(
                os.path.join(self.config.screenshots_dir, filename)
            )
    ...
    def check_session_validity(self) -> bool:
        ...
        try:
            ...
            if (
                any(indicator in page_text for indicator in login_indicators)
                and "logout" not in page_text
            ):
    ...
    def recover_session(self) -> bool:
        ...
        if self.session_recovery_attempts >= self.config.max_session_recovery_attempts:
            self.logger.error(
                f"❌ Max session recovery attempts ({self.config.max_session_recovery_attempts}) exceeded"
            )
            return False
    ...
    def adjust_delays(self, success: bool):
        ...
        if success:
            ...
            self.current_delays["action"] = max(
                0.5, self.current_delays["action"] * 0.95
            )
            self.current_delays["short"] = max(0.3, self.current_delays["short"] * 0.95)
        else:
            # Slow down on failure
            self.current_delays["action"] = min(
                5.0, self.current_delays["action"] * 1.2
            )
    ...
    def close_swal_modal(self, button_texts: List[str] = None) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"   📦 SweetAlert found with {swal_info.get('hasButtons')} button(s)"
            )
    ...
    def detect_current_phase(self) -> Tuple[FormPhase, Dict]:
        ...
        try:
            ...
            if swal_info.get("visible"):
                ...
                if any(
                    x in swal_text
                    for x in ["pasti", "adakah anda pasti", "confirm", "are you sure"]
                ):
                    return FormPhase.PHASE_5_CONFIRM, {
                        "method": "swal_quick",
                        "swal_text": swal_text[:100],
                    }
                # Check for success
                if any(
                    x in swal_text
        ...
        if (
            self.config.use_ai_verification
            and self.ai_failures < self.max_ai_failures
            and self.page_analyzer
        ):
            try:
        ...
        if self.ai_failures >= self.max_ai_failures:
            if self.config.use_ai_verification:
                self.logger.warning(
                    f"⚠️ Switching to simple detection (AI failed {self.ai_failures}x)"
                )
                self.config.use_ai_verification = False
    ...
    def attempt_ai_recovery(
        self, error: Exception, expected_action: str, phase: str
    ) -> bool:
        ...
        try:
            ...
            analysis = self.error_analyzer.analyze_error(
                self.page, str(error), expected_action, phase
            )
            ...
            self.logger.info(
                f"   📋 Executing {len(recovery_actions)} recovery steps..."
            )

            for i, action in enumerate(recovery_actions):
                self.logger.info(
                    f"   Step {i + 1}/{len(recovery_actions)}: {action.get('action', 'unknown')}"
                )
            ...
            if detected_phase != FormPhase.UNKNOWN:
                self.logger.info(
                    f"   ✅ Recovery successful! Now on: {detected_phase.value}"
                )
                return True
            else:
    ...
    def check_for_duplicate(self, book: Dict) -> Tuple[bool, str]:
        ...
        if self.config.use_ai_verification and self.duplicate_detector:
            try:
                is_dup, reason = self.duplicate_detector.check_if_exists(
                    self.page, book
                )
                if is_dup:
    ...
    def click_button_safe(
        self, button_texts: List[str], wait_after: float = None, force_js: bool = False
    ) -> bool:
        """Safely click button by text."""
        wait_time = (
            wait_after if wait_after is not None else self.current_delays["action"]
        )
        self.close_modal_if_present()
        ...
        try:
            ...
            if self.config.add_to_favorites:
                ...
                if success:
                    ...
                    self.logger.warning(
                        "   ⚠️ Could not detect add button - modal might auto-close"
                    )
            else:
                ...
                if success:
                    ...
                    if self.close_swal_modal(["OK"]):
                        ...
                        self.logger.warning(
                            "   ⚠️ Could not detect skip button - modal might auto-close"
                        )
    ...
    def navigate_to_new_book_form(self) -> bool:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                self.logger.debug(
                    f"   Attempt {attempt + 1}: Current phase = {detected_phase.value}"
                )
                ...
                if detected_phase == FormPhase.SUCCESS:
                    ...
                    for btn in nav_buttons:
                        if self.click_button_safe([btn], force_js=True):
                            self.logger.debug(
                                f"   Clicked '{btn}', waiting for navigation..."
                            )
                    ...
                    self.logger.debug(
                        "   On confirm page during navigation, skipping..."
                    )
                    # Don't try to interact, just wait and let page settle
                    time.sleep(self.current_delays["action"])
                    continue

                elif detected_phase in [
                    FormPhase.PHASE_3_EMPTY,
                    FormPhase.PHASE_4_SUBMIT,
                ]:
                    ...
                    if not nav_tried:
                        ...
                        if menu_clicked:
                            self.logger.debug(
                                "   Clicked menu item, waiting for page load..."
                            )
                            time.sleep(self.current_delays["action"])
                            try:
                                self.page.wait_for_load_state(
                ...
                self.logger.warning(
                    f"   ⚠️  Navigation error (attempt {attempt + 1}): {e}"
                )
                time.sleep(self.current_delays["short"])
        ...
        self.logger.error(
            f"   ❌ Could not navigate to new book form (stuck at: {detected_phase.value})"
        )
        return False
    ...
    def handle_book_type_selection(self) -> bool:
        ...
        try:
            ...
            if detected_phase != FormPhase.BOOK_TYPE_SELECT:
                self.logger.debug(
                    f"   ℹ️ Not on book type page (on {detected_phase.value}), skipping"
                )
                return True
        ...
        try:
            ...
            if not is_valid:
                ...
                self.take_screenshot(
                    f"phase1_validation_fail_{self.current_book_index}"
                )
            ...
            if self.attempt_ai_recovery(
                e, "complete Phase 1 basic info form", "PHASE_1_BASIC"
            ):
                return True
            raise
    ...
    def handle_phase_2_summary(self, book: Dict) -> bool:
        ...
        try:
            phase_data = {
                "rumusan": book.get(
                    "summary", book.get("synopsis", book.get("rumusan", ""))
                ),
            ...
            if not is_valid:
                self.logger.warning(f"   ⚠️ Validation issues: {issues}")
                self.take_screenshot(
                    f"phase2_validation_fail_{self.current_book_index}"
                )
            ...
            if self.attempt_ai_recovery(
                e, "complete Phase 2 summary form", "PHASE_2_SUMMARY"
            ):
                return True
            raise
    ...
    def handle_phase_4_submit(self) -> bool:
        ...
        try:
            ...
            if not is_valid:
                ...
                self.take_screenshot(
                    f"phase4_validation_fail_{self.current_book_index}"
                )

            if not self.config.dry_run:
                if not self.click_button_safe(
                    ["Hantar", "Submit"], wait_after=1.5, force_js=True
                ):
    ...
    def handle_phase_5_confirm(self) -> bool:
        ...
        try:
            if not self.config.dry_run:
                ...
                if detected_phase in [FormPhase.SUCCESS, FormPhase.FAVORITES]:
                    self.logger.info(
                        f"   ℹ️ Already on {detected_phase.value} (auto-confirmed)"
                    )
                    self.book_submitted = True
                    return True
            ...
            if self.attempt_ai_recovery(
                e, "click Pasti confirmation button", "PHASE_5_CONFIRM"
            ):
                self.book_submitted = True
                return True
            raise
    ...
    def handle_success_and_navigation(self) -> bool:
        ...
        if self.duplicate_tracker and self.current_book:
            self.duplicate_tracker.mark_as_processed(
                self.current_book, self.current_book_index
            )
        ...
        while modal_attempts < 5:
            ...
            self.logger.debug(
                f"   Post-success phase check (attempt {modal_attempts}): {detected_phase.value}"
            )
            ...
            if detected_phase == FormPhase.SUCCESS:
                ...
                FormPhase.BOOK_TYPE_SELECT,
                FormPhase.PHASE_1_BASIC,
            ]:
        ...
        self.logger.warning(
            f"⚠️ Post-success state unclear ({detected_phase.value}), attempting navigation..."
        )
    ...
    def _fill_dropdowns(self, book: Dict) -> bool:
        try:
            ...
            detected_language = LanguageDetector.detect(
                book, self.config.default_language
            )
            ...
            for i in range(count):
                try:
                    ...
                    # Check if this is a language dropdown by identifier OR by option values
                    lang_options = ["English", "Bahasa Melayu", "Bahasa Cina", "Bahasa"]
                    has_lang_options = any(
                        opt["text"] in lang_options or opt["value"] in lang_options
                        for opt in options
                    )
                    is_lang_dropdown = (
                        any(x in identifier for x in ["bahasa", "language", "lang"])
                        or has_lang_options
                    )

                    if is_lang_dropdown:
                        # Map detected language to what AINS dropdown actually has
                        ains_lang = LanguageDetector.get_ains_dropdown_value(
                            detected_language
                        )
                        self.logger.debug(
                            f"   Detected: {detected_language} → AINS dropdown: {ains_lang}"
                        )
                        # Deterministic override: force English for English-detected books
                        if detected_language == "English" and ains_lang != "English":
                            self.logger.debug(
                                "Forcing AINS dropdown to English for English book"
                            )
                            ains_lang = "English"
                        self.logger.debug(
                            f"   🌐 Detected: {detected_language} → AINS dropdown: {ains_lang}"
                        )
                        selected = self._select_option_fuzzy(select, options, ains_lang)
                        if not selected:
                            # Fallback: try the raw detected language name
                            selected = self._select_option_fuzzy(
                                select, options, detected_language
                            )
                        if selected:
                            self.logger.debug(f"   ✓ Language set: {ains_lang}")
                        else:
                            self.logger.warning(
                                f"   ⚠️ Could not set language to {ains_lang}"
                            )

                    elif any(
                        x in identifier
                        for x in ["kategori", "category", "jenis", "type", "genre"]
                    ):
                        book_category = book.get(
                            "category", book.get("genre", self.config.default_category)
                        )
                        # Normalize category to match dropdown options
                        book_category_norm = self._normalize_category(book_category)
                        selected = self._select_option_fuzzy(
                            select, options, book_category_norm
                        )
                        if selected:
                            self.logger.debug(f"   ✓ Category: {book_category}")

                    else:
                        # Select first non-empty option as a baseline
                        try:
                            if len(options) > 1:
                            ...
                            pass
                        # Additional fallback for category selection: match on option text
                        # Recompute a reasonable category if not yet determined
                        book_category = book.get(
                            "category", book.get("genre", self.config.default_category)
                        )
                        book_category_norm = self._normalize_category(book_category)
                        if not selected:
                            target = str(book_category_norm).strip().lower()
                            for opt in options:
                                opt_text = str(opt.get("text", "")).lower().strip()
                                if target == opt_text or target in opt_text:
                                    try:
                                        select.select_option(
                                            value=opt["value"], timeout=2000
                                        )
                                        selected = True
                                        break
                                    except:
    ...
    def process_book(self, book: Dict, index: int, tracker: ProgressTracker) -> bool:
        ...
        self.logger.info("=" * 70)
        self.logger.info(
            f"📖 {progress} {title[:50]}{'...' if len(title) > 50 else ''}"
        )
        ...
        try:
            ...
            if self.config.check_for_duplicates:
                ...
                if is_dup:
                    if self.config.skip_duplicates:
                        ...
                        self.logger.warning(
                            f"⚠️  Possible duplicate: {reason} (continuing anyway)"
                        )
            ...
            if not self.handle_success_and_navigation():
                ...
                if self.book_submitted:
                    self.logger.warning(
                        "⚠️ Navigation issue after success (book was saved)"
                    )
            ...
            if self.book_submitted:
                self.logger.info(
                    "   ℹ️ Book appears to have been submitted before error"
                )
...
# ===========================
def load_customers(config: Config) -> Dict:
    """Load customer data from customers.json."""
    path = config.get_path(config.customers_file)
...
def save_customers(config: Config, data: Dict):
    ...
    try:
        ...
        print(f"Warning: Could not save customers: {e}")


def get_customer_progress_file(customer_id: str) -> str:
    """Get per-customer progress filepath."""
    return f"data/{customer_id}/progress.json"


def get_customer_duplicates_file(customer_id: str) -> str:
    """Get per-customer duplicates filepath."""
    return f"data/{customer_id}/duplicates.json"


def get_customer_report_dir(customer_id: str) -> str:
    """Get per-customer reports directory."""
    return f"reports/{customer_id}"


def ensure_customer_dirs(config: Config, customer_id: str):
    """Create data and report directories for a customer."""
    os.makedirs(config.get_path(f"data/{customer_id}"), exist_ok=True)
    os.makedirs(config.get_path(f"reports/{customer_id}"), exist_ok=True)


def show_customer_menu(config: Config, logger: logging.Logger) -> Optional[str]:
    """Show customer selection menu. Returns customer_id or None to exit."""
    customers_data = load_customers(config)
    ...
    if customers:
        print("\n  Existing customers:")
        print("  " + "-" * 50)
        sorted_customers = sorted(customers.items(), key=lambda x: x[0])
        for cid, info in sorted_customers:
            name = info.get("name", "Unknown")
            username = info.get("username", "")
            limit = info.get("daily_limit", 0)
            last_book = info.get("last_completed_index", -1)
            total_done = last_book + 1
            user_str = f" ({username})" if username else ""
            limit_str = "unlimited" if limit == 0 else f"{limit}/day"
            print(f"    [{cid}] {name}{user_str} — {total_done} done, {limit_str}")
        print("  " + "-" * 50)
    ...
    while True:
        ...
        if choice.upper() == "N":
            ...
            # Generate ID
            next_id = customers_data.get("next_id", 1)
            new_id = f"{next_id:03d}"

            # Ask for AINS username
            username = safe_input("   AINS login username (e.g. customer001): ")
            if not username or not username.strip():
                username = ""
            else:
                username = username.strip()

            # Ask for starting book
            start_from = safe_input("   Start from book number (default=1): ")
            start_idx = 0
            if start_from and start_from.strip():
                try:
                    start_idx = max(0, int(start_from.strip()) - 1)
                except ValueError:
                    print("   ⚠️  Invalid number, starting from book 1")
                    start_idx = 0

            # Ask for daily submission limit
            limit_input = safe_input("   Daily book limit (0=unlimited): ")
            daily_limit = 0
            if limit_input and limit_input.strip():
                try:
                    daily_limit = max(0, int(limit_input.strip()))
                except ValueError:
                    print("   ⚠️  Invalid number, using unlimited")
            ...
            customers[new_id] = {
                ...
                "last_completed_index": start_idx - 1 if start_idx > 0 else -1,
                "created": datetime.now().isoformat(),
            }
            customers_data["customers"] = customers
            customers_data["next_id"] = next_id + 1
            save_customers(config, customers_data)

            # Create customer directories
            ensure_customer_dirs(config, new_id)

            # Initialize their progress file
            progress_file = get_customer_progress_file(new_id)
            progress_path = config.get_path(progress_file)
            if not os.path.exists(progress_path):
                ...
                # Pre-fill skipped entries for books before start_idx
                if start_idx > 0:
                    for i in range(start_idx):
                        initial_data["books"][str(i)] = {
                            "title": f"Book {i + 1}",
                            "status": "success",
                            "error": None,
                            "timestamp": datetime.now(UTC).isoformat() + "Z",
                        }
                    initial_data["stats"]["success"] = start_idx

                with open(progress_path, "w", encoding="utf-8") as f:
                    json.dump(initial_data, f, indent=2, ensure_ascii=False)

            logger.info(
                f"✅ Created customer [{new_id}] {name}, starting from book {start_idx + 1}"
            )
            print(f"\n   ✅ Customer [{new_id}] created!")
            return new_id

        elif choice.upper() == "D" and customers:
            del_id = safe_input("   Enter customer ID to delete: ")
            if del_id and del_id.strip() in customers:
                del_name = customers[del_id.strip()].get("name", "Unknown")
                confirm = safe_confirm(
                    f"   Delete customer [{del_id.strip()}] {del_name}? (y/n): "
                )
                if confirm:
                    del customers[del_id.strip()]
                    customers_data["customers"] = customers
                    save_customers(config, customers_data)
                    print(f"   ✅ Customer [{del_id.strip()}] deleted")
                else:
                    print("   Cancelled")
            else:
            ...
            print(f"   ⚠️  Invalid choice. Enter a customer ID, N, or 0")
...
def print_banner():
    print("\n" + "=" * 70)
    print("  📚 AINS BOOK AUTOMATION v5.3 - BUSINESS EDITION")
    print("  " + "-" * 66)
    print("  Automated book entry for AINS library system")
    print("  ")
    print("  🆕 NEW IN v5.3:")
    print("    ✓ Multi-customer support — track progress per customer")
    print("    ✓ Customer selection menu at startup")
    print("    ✓ Per-customer progress files")
    print("    ✓ Unlimited processing by default (no 30-book cap)")
    print("    ✓ Duplicate detection disabled (each customer needs all books)")
    print("  ")
    print("  🔧 INHERITED:")
    print("    ✓ Multi-language support (Malay, English, Chinese)")
    print("    ✓ AI error recovery, adaptive delays")
    print("    ✓ SweetAlert/Pasti detection, favorites")
    print("=" * 70 + "\n")


def show_startup_menu(
    tracker: ProgressTracker, books: list, logger: logging.Logger
) -> Tuple[int, bool]:
    """
    ...
    if stale_count > 0:
        logger.warning(
            f"Found {stale_count} submitted entries outside current book list; ignoring them"
        )
    ...
    if submitted_books:
        ...
        while True:
            ...
            if choice == "1":
                # Fresh start - reset everything (with confirmation)
                if not safe_confirm(
                    "⚠️  This will erase ALL progress for this customer. Are you sure? (y/n): "
                ):
                    print("   Cancelled.")
                    continue
                tracker.reset()
                ...
                logger.info(
                    f"📍 Starting from beginning, will skip {len(submitted_books)} submitted books"
                )
                print(
                ...
                # Custom start index
                custom = safe_input(
                    f"   Enter book number to start from (1-{total_books}): "
                )
                if custom is None:
                    continue
                try:
                    ...
                    if 0 <= custom_idx < total_books:
                        skip = safe_confirm(
                            "   Skip already submitted books? (y/n): ", default=True
                        )
                        if skip:
                            tracker.reset_keep_submitted()
                        logger.info(
                            f"📍 Custom start from Book #{custom_idx + 1}, skip={skip}"
                        )
...
def main():
    ...
    # --- Initialize default customers if customers.json doesn't exist ---
    if not os.path.exists(config.get_path(config.customers_file)):
        default_customers = {
            "customers": {
                "001": {
                    "name": "Customer 001",
                    "last_completed_index": 89,
                    "created": datetime.now().isoformat(),
                },
                "002": {
                    "name": "Customer 002",
                    "last_completed_index": 29,
                    "created": datetime.now().isoformat(),
                },
                "003": {
                    "name": "Customer 003",
                    "last_completed_index": 119,
                    "created": datetime.now().isoformat(),
                },
            },
            "next_id": 4,
        }
        save_customers(config, default_customers)

        # Create per-customer progress files
        for cid, info in default_customers["customers"].items():
            ensure_customer_dirs(config, cid)
            last_idx = info["last_completed_index"]
            progress_data = {
            ...
            # Pre-fill completed books
            for i in range(last_idx + 1):
                progress_data["books"][str(i)] = {
                    "title": f"Book {i + 1}",
                    "status": "success",
                    "error": None,
                    "timestamp": datetime.now(UTC).isoformat() + "Z",
                }
            pf = config.get_path(get_customer_progress_file(cid))
            with open(pf, "w", encoding="utf-8") as f:
                json.dump(progress_data, f, indent=2, ensure_ascii=False)

        logger.info(
            "✅ Initialized 3 customers: 001 (90 books), 002 (30 books), 003 (120 books)"
        )

    # --- Customer Selection ---
    customer_id = show_customer_menu(config, logger)
    if customer_id is None:
        logger.info("No customer selected, exiting.")
        return

    # Set per-customer progress/duplicates files
    config.customer_id = customer_id
    config.progress_file = get_customer_progress_file(customer_id)
    config.duplicates_file = get_customer_duplicates_file(customer_id)
    config.skip_duplicates = False  # Don't skip - each customer needs all books
    ensure_customer_dirs(config, customer_id)

    customers_data = load_customers(config)
    cust_info = customers_data.get("customers", {}).get(customer_id, {})
    customer_name = cust_info.get("name", "Unknown")
    customer_daily_limit = cust_info.get("daily_limit", 0)

    automation = BookAutomation(config, logger)

    logger.info(
        f"🚀 Starting AINS Book Automation v5.3 — Customer [{customer_id}] {customer_name}"
    )
    ...
    available_files = {}
    for lang_key, filename in config.json_files.items():
        filepath = config.get_path(filename)
        if os.path.exists(filepath):
            try:
                with open(filepath, "r", encoding="utf-8") as f:
                    count = len(json.load(f))
                available_files[lang_key] = (filename, count)
            except (json.JSONDecodeError, Exception):
                pass

    if not available_files:
        # Fallback: try the single default file
        logger.warning("No configured JSON files found, trying default...")
        ...
        print("\n" + "=" * 70)
        print("📂 BOOK SOURCE SELECTION")
        print("-" * 70)

        keys_list = list(available_files.keys())
        for idx, lang_key in enumerate(keys_list, 1):
            filename, count = available_files[lang_key]
            lang_display = lang_key.capitalize()
            print(f"   [{idx}] {lang_display:12s} — {filename} ({count} books)")

        all_count = sum(c for _, c in available_files.values())
        print(f"   [A] {'All':12s} — All files combined ({all_count} books)")
        print("   [0] Exit")
        print("=" * 70)

        while True:
            choice = safe_input("\n>>> Select book source (number/A/0): ")
            if choice is None or choice.strip() == "0":
            ...
            if choice == "A":
                logger.info("📚 Loading ALL book files...")
                books = automation.load_books_multi()
                # Use default progress/duplicates files for combined source
                config.progress_file = get_customer_progress_file(customer_id)
                config.duplicates_file = get_customer_duplicates_file(customer_id)
                selected_source = "all"
                break
            ...
            try:
                ...
                if 0 <= choice_idx < len(keys_list):
                    selected_key = keys_list[choice_idx]
                    selected_file = available_files[selected_key][0]
                    logger.info(
                        f"📚 Loading {selected_key} books from {selected_file}..."
                    )
                    config.json_file = selected_file  # Update active file
                    books = automation.load_books(selected_file)
                    # Use source-specific progress/duplicates files
                    config.progress_file = f"data/{customer_id}/progress_{selected_key}.json"
                    config.duplicates_file = f"data/{customer_id}/duplicates_{selected_key}.json"
                    selected_source = selected_key
                    break
                else:
                    print(f"   ⚠️  Enter 1-{len(keys_list)}, A, or 0")
            except ValueError:
                print(f"   ⚠️  Enter 1-{len(keys_list)}, A, or 0")

        if not books:
            logger.error("❌ No valid books to process.")
            return

    # Now create tracker with the correct progress file
    tracker = ProgressTracker(config, logger)

    # Handle reset after tracker is created
    ...
    for lang, count in sorted(lang_stats.items(), key=lambda x: -x[1]):
        ains_mapped = LanguageDetector.get_ains_dropdown_value(lang)
        if ains_mapped != lang:
            logger.info(f"   {lang}: {count} books → AINS dropdown: {ains_mapped}")
        else:
    ...
    if skip_submitted:
        completed_count = sum(
            1 for i in range(start_index, len(books)) if tracker.is_book_completed(i)
        )
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print(f"🔐 CUSTOMER [{customer_id}] {customer_name} — LOGIN & SETUP")
        print("-" * 70)
        customer_username = (
            customers_data.get("customers", {}).get(customer_id, {}).get("username", "")
        )
        if customer_username:
            print(f"   👤 Log in as: {customer_username}")
        ...
        print("")
        print(
            f"💡 Customer [{customer_id}] progress: Book #{tracker.data['last_completed_index'] + 2} of {len(books)}"
        )
        ...
        if customer_daily_limit > 0:
            print(f"   ✓ Daily limit: {customer_daily_limit} books")
        else:
        ...
        # Count today's submissions (calendar day, not session)
        today_str = datetime.now(UTC).date().isoformat()
        books_processed = sum(
            1
            for v in tracker.data["books"].values()
            if v.get("status") == "success"
            and v.get("timestamp", "").startswith(today_str)
        )
        if books_processed > 0:
            if customer_daily_limit > 0:
                logger.info(
                    f"📅 Already submitted {books_processed} books today (limit: {customer_daily_limit})"
                )
            else:
                logger.info(
                    f"📅 Already submitted {books_processed} books today (unlimited)"
                )
        if customer_daily_limit > 0 and books_processed >= customer_daily_limit:
            logger.warning(
                f"🛑 Daily limit already reached ({books_processed}/{customer_daily_limit}). Come back tomorrow!"
            )
            return
        ...
        while i < len(books):
            ...
            if tracker.is_book_completed(i):
                logger.info(
                    f"⏭️  Skipping already completed: [{i + 1}/{len(books)}] {title[:40]}"
                )
                i += 1
                continue
            ...
            try:
                ...
                if success:
                    # Check if process_book already marked it (e.g., as DUPLICATE or SUCCESS)
                    existing = tracker.data["books"].get(str(i), {}).get("status")
                    if existing == BookStatus.DUPLICATE.value:
                        logger.info(
                            "   ⏭️  Duplicate handled, not counting against daily limit"
                        )
                        consecutive_failures = 0
                    elif existing == BookStatus.SUCCESS.value:
                        # Already marked success by process_book
                        consecutive_failures = 0
                        books_processed += 1
                    else:
                    ...
                    if (
                        customer_daily_limit > 0
                        and books_processed >= customer_daily_limit
                    ):
                        logger.info(
                            f"\n🛑 AINS daily limit reached for [{customer_id}] {customer_name} ({customer_daily_limit} books)"
                        )
                        tracker.save()

                        # Offer to switch to another customer
                        all_cust = load_customers(config).get("customers", {})
                        other_custs = {
                            cid: c for cid, c in all_cust.items() if cid != customer_id
                        }

                        if other_custs:
                            print("\n" + "=" * 50)
                            print("  Switch to another customer?")
                            print("  " + "-" * 40)
                            for cid, cinfo in other_custs.items():
                                cname = cinfo.get("name", "Unknown")
                                cdone = cinfo.get("last_completed_index", -1) + 1
                                cuser = cinfo.get("username", "")
                                user_str = f" ({cuser})" if cuser else ""
                                print(
                                    f"    [{cid}] {cname}{user_str} — {cdone} books done"
                                )
                            print(f"    [0]  Exit")
                            print("=" * 50)

                            switch = safe_input("\n>>> Select customer or 0 to exit: ")
                            if switch and switch.strip() in other_custs:
                                # Switch to new customer
                                customer_id = switch.strip()
                                config.customer_id = customer_id
                                config.progress_file = get_customer_progress_file(
                                    customer_id
                                )
                                config.duplicates_file = get_customer_duplicates_file(
                                    customer_id
                                )
                                ensure_customer_dirs(config, customer_id)
                                customer_name = other_custs[customer_id].get(
                                    "name", "Unknown"
                                )
                                customer_daily_limit = other_custs[customer_id].get(
                                    "daily_limit", 0
                                )

                                # Reload tracker for new customer
                                tracker = ProgressTracker(config, logger)
                                ...
                                if books_processed > 0:
                                    logger.info(
                                        f"📅 [{customer_id}] already submitted {books_processed} today"
                                    )
                                if (
                                    customer_daily_limit > 0
                                    and books_processed >= customer_daily_limit
                                    ...
                                    logger.warning(
                                        f"🛑 [{customer_id}] also at daily limit!"
                                    )
                                    break

                                # Reset loop to start from new customer's position
                                start_index = tracker.get_start_index()
                                i = start_index
                                consecutive_failures = 0
                                logger.info(
                                    f"\n🔄 Switched to [{customer_id}] {customer_name}. Resuming from book #{i + 1}"
                                )
                                continue
                            else:
                                break
                    ...
                    if book_status == BookStatus.DUPLICATE.value:
                        ...
                        tracker.mark_book(
                            i, title, BookStatus.FAILED, "Processing failed"
                        )
                        consecutive_failures += 1
                        logger.warning(
                            f"   ⚠️  Failed (consecutive: {consecutive_failures}), continuing..."
                        )
                ...
                if consecutive_failures >= max_consecutive_failures:
                    ...
                    if consecutive_failures % max_consecutive_failures == 0:
                        ...
                        if not safe_confirm(
                            "Continue anyway? (y/n, default=y): ", default=True
                        ):
                            logger.info("Stopping due to failures")
                            break

                i += 1  # Move to next book
                ...
                if choice == "1":
                    ...
                    # Reset page state before retrying
                    try:
        ...
        print("\n")
        logger.info("=" * 70)
        logger.info(f"📊 CUSTOMER [{customer_id}] {customer_name} — SESSION COMPLETE")
        logger.info("-" * 70)
        ...
        logger.info(
            f"   📚 Total:       {stats['success'] + stats['failed'] + stats['duplicate'] + stats['skipped']}"
        )
        logger.info(
            f"   📍 Last completed: Book #{tracker.data['last_completed_index'] + 1} of {len(books)}"
        )
        logger.info("-" * 70)
        ...
        try:
            today_str = datetime.now(UTC).date().isoformat()
            report_dir = config.get_path(f"reports/{customer_id}")
            os.makedirs(report_dir, exist_ok=True)
            report_name = f"report_{today_str}.txt"
            report_path = os.path.join(report_dir, report_name)

            # Collect today's submissions and failures
            submitted_today = []
            failed_today = []
            for key, v in tracker.data["books"].items():
                if v.get("timestamp", "").startswith(today_str):
                    if v.get("status") == "success":
                        submitted_today.append(
                            (int(key), v.get("title", "?"), v.get("timestamp", ""))
                        )
                    elif v.get("status") == "failed":
                        failed_today.append(
                            (int(key), v.get("title", "?"), v.get("error", "unknown"))
                        )
            submitted_today.sort()
            failed_today.sort()

            with open(report_path, "w", encoding="utf-8") as f:
                f.write("=" * 50 + "\n")
                f.write("AINS Book Entry Report\n")
                f.write("=" * 50 + "\n")
                f.write(f"Date:     {today_str}\n")
                f.write(f"Customer: {customer_id} ({customer_name})\n")
                f.write(f"Submitted: {len(submitted_today)}\n")
                f.write(f"Failed:    {len(failed_today)}\n")
                limit_str = (
                    "Unlimited"
                    if customer_daily_limit == 0
                    else f"{customer_daily_limit}/day"
                )
                f.write(f"Limit:     {limit_str}\n")
                f.write("\n")

                if submitted_today:
                    f.write("--- Books Submitted ---\n")
                    for idx, title, ts in submitted_today:
                        f.write(f"  [{idx + 1}] {title}\n")
                    f.write("\n")

                if failed_today:
                    f.write("--- Books Failed ---\n")
                    for idx, title, err in failed_today:
                        f.write(f"  [{idx + 1}] {title} — {err}\n")
                    f.write("\n")

                f.write(
                    f"Next session: Book #{tracker.data['last_completed_index'] + 2}\n"
                )
                remaining = len(books) - tracker.data["last_completed_index"] - 1
                f.write(f"Remaining: {remaining} books\n")
